<template>
  <component :is="item.component"
             ...
             v-bind="item.props"/>
  <div v-else-if="item.header && !isItemHidden"
       class="vsm--header"
       :class="item.class"
  ...
  <div v-else-if="!isItemHidden"
    ...
    <template v-if="isRouterLink">
      <router-link :class="itemLinkClass"
        ...
        <template v-if="item.icon">
          <i v-if="typeof item.icon === 'string' || (item.icon instanceof String)"
             class="vsm--icon"
             :class="item.icon"/>
    ...
    <template v-else>
      <component :is="itemLinkHref ? 'a' : 'span'"
                 :class="itemLinkClass"
      ...
      <template v-if="(isCollapsed && !isFirstLevel) || !isCollapsed">
        <transition name="expand"
          ...
          <div v-if="show"
               class="vsm--dropdown">
            <div class="vsm--list">
              <sidebar-menu-item v-for="(subItem, index) in item.child"
                                 :key="index"
                                 :item="subItem"
                                 :level="level+1"
...
<script>
import { itemMixin, animationMixin } from '../../helpers/mixin'
export default {
